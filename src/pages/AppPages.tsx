...
function sceneText(value: string) { const endpoint = String(value || '').toLowerCase(); return endpoint.includes('workflow') ? '流程图生成' : endpoint.includes('compile') ? '生成自定义策略' : endpoint.includes('diag') ? '开仓诊断' : endpoint.includes('position') || endpoint.includes('risk') ? '持仓风控' : endpoint.includes('open') ? '开单分析' : value || '-' }
// Strategies whose server side pyramids. Every other strategy holds a single
// position and ignores the position-limit / add-on settings, so the client must
// not offer those controls for them.
const ADDING_STRATEGIES = new Set(['GL_TREND_V1', 'CUSTOM_AI_V1'])
function strategySupportsAdding(code: string | undefined | null) { return ADDING_STRATEGIES.has(String(code || '')) }
...
function StrategyForm({ source, editing = false, deployment, onSaved, onCancel }: { source?: 'official' | 'custom'; editing?: boolean; deployment?: PortalStrategy; onSaved?: () => void; onCancel?: () => void }) {
  ...
  type OfficialStrategyOption = { code: string; name: string; summary: string; open_ai_endpoint_id: string; position_ai_endpoint_id: string; default_config: StrategyDefaultConfig }
  const [libraryStrategies, setLibraryStrategies] = useState<OfficialStrategyOption[]>([])
  const [libraryStrategy, setLibraryStrategy] = useState<OfficialStrategyOption | null>(null)
  ...
  useEffect(() => {
    ...
    apiRequest<{ list: Array<{ code: string; name: string; summary: string; open_ai_endpoint_id: string; position_ai_endpoint_id: string; default_config: StrategyDefaultConfig }> }>('/api/v1/auth/official-strategies')
      .then((result) => {
        ...
        setLibraryStrategies(result.list || [])
  ...
  useEffect(() => {
    ...
    const isGlTrend = libraryStrategy.code === 'GL_TREND_V1'
    ...
    setMaxPositions(String(config.max_positions ?? (isGlTrend ? 4 : 1)))
    setAllowAdd(Boolean(config.allow_add_position ?? config.allow_add ?? isGlTrend))
  ...
  // Only strategies that actually pyramid expose the position-limit and add-on
  // controls. PA_AGENT_V1 holds a single position and its server side ignores
  // both settings, so showing them would offer a switch that does nothing.
  const supportsAdding = strategySupportsAdding(strategySource === 'custom' ? 'CUSTOM_AI_V1' : (libraryStrategy?.code || deployment?.strategy_code))
  ...
  return <form noValidate className={`strategy-form ${strategySource === 'custom' ? 'custom-strategy-form' : 'library-strategy-form'}`} onSubmit={submit} aria-busy={saving}>
    ...
    {strategySource === 'official' ? <section className="panel form-section strategy-library-section"><div className="form-section-title"><span>01</span><div><h2>选择GL策略</h2><p>当前可选择已经配置完成的 GL 策略。</p></div></div>{libraryStrategies.map((item) => <label key={item.code} className={`library-option ${libraryStrategy?.code === item.code ? 'selected' : ''}`} onClick={() => setLibraryStrategy(item)}><input type="radio" name="official-strategy" checked={libraryStrategy?.code === item.code} onChange={() => setLibraryStrategy(item)} /><strong>{item.name}</strong><button className="library-detail-button" type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); setLibraryStrategy(item); setStrategyDetailOpened(true) }}>查看详情<ChevronRight size={15} /></button>{libraryStrategy?.code === item.code && <Check />}</label>)}</section> : <>
