...
function CreateStrategyAction() {
  ...
  if (!user.vip_active) return <button className="button button-primary" type="button" disabled title="VIP 未开通或已到期，暂时不能创建策略"><Plus size={17} />创建策略</button>
  return <Link className="button button-primary" to="/app/strategies/new"><Plus size={17} />创建策略</Link>
}
...
export function DashboardPage() {
  ...
  return <>
    <PageHeading eyebrow="OVERVIEW" title={`晚上好，${displayName}`} description="这里是您的策略运行和 AI 余额概览。" action={<CreateStrategyAction />} />
...
export function WalletPage() {
  ...
  const warningThreshold = Number(wallet.low_balance_threshold || 10).toLocaleString('zh-CN', { maximumFractionDigits: 2 })
  return <>
    <PageHeading eyebrow="AI WALLET" title="AI 余额" description="使用官方 AI 时按各模型实际输入、输出 Token 价格实时计费；自定义 AI 不扣除平台余额。" />
    {usesOfficialAi && wallet.balance_warning && <div className="permission-notice balance-warning-danger"><Coins size={21} /><div><strong>当前余额不足 {warningThreshold} 元，为保证 EA 正常运行，建议及时充值</strong><span>账户余额 ¥{money(wallet.balance)}，含信用额度当前可用 ¥{money(wallet.available_balance)}。{wallet.credit_exhausted ? '信用额度已用完，官方 AI 已停止调用。' : ''}</span></div></div>}
    <section className="wallet-grid"><article className="wallet-card"><div className="wallet-card-top"><div><span>当前账户余额</span><strong>¥{money(wallet.balance)}</strong><small>人民币 CNY</small></div><WalletCards /></div><div className="wallet-card-bottom"><div><span>含信用额度可用</span><strong>¥{money(wallet.available_balance)}</strong></div><div><span>信用额度</span><strong>¥{money(wallet.credit_limit)}</strong></div></div></article><article className="panel recharge-card"><div className="panel-heading"><div><span className="eyebrow">MANUAL TOP-UP</span><h2>人工充值</h2></div><CreditCard /></div><p>当前使用人工转账充值。完成转账后联系管理员，管理员确认后将金额加入您的 AI 账户余额。</p><div className="recharge-notice"><ShieldCheck /><span><strong>每次余额变动都会生成流水</strong><small>累计入账 ¥{money(wallet.total_credit)}，累计扣减 ¥{money(wallet.total_debit)}。</small></span></div><button className="button button-secondary" type="button">查看充值说明</button></article></section>
...
export function UsagePage() {
  ...
  return <>
    ...
    <section className="panel"><div className="panel-heading"><div><span className="eyebrow">CALL HISTORY</span><h2>调用明细</h2></div><span className="usage-retention-badge">仅保留 {retentionDays} 天</span></div>
