...
function ConditionFields({ condition, visionSources, onChange }: { condition?: WorkflowCondition; visionSources: WorkflowNode[]; onChange: (value: WorkflowCondition) => void }) {
  ...
  return <>
    ...
    {condition.kind !== 'vision_result' && <label><span>条件说明</span><textarea rows={3} value={condition.description || ''} onChange={(event) => patch({ description: event.target.value })} /></label>}
