...
function IndicatorFields({ value, onChange, title, catalog, compatibleGroups, conditionKind }: { value: WorkflowOperand; onChange: (value: WorkflowOperand) => void; title: string; catalog: IndicatorCatalogItem[]; compatibleGroups?: string[]; conditionKind?: string }) {
  ...
  return <fieldset className="workflow-fieldset"><legend>{title}</legend>{value.kind === 'position' && <button type="button" className="workflow-add-output" onClick={() => onChange({ ...value, name: 'price_sl_distance' })}>选择最新价 - 止损价</button>}
...
function FormSelect({ value, data, onChange }: { value: string; data: Array<[string, string]>; onChange: (value: string) => void }) {
  const options = data.some(([itemValue]) => itemValue === 'side') && !data.some(([itemValue]) => itemValue === 'price_sl_distance')
...
function OperandFields({ value, onChange, title, catalog, allowConstant = true, allowedKinds, compatibleGroups, conditionKind, positionSideConstant = false }: { value: WorkflowOperand; onChange: (value: WorkflowOperand) => void; title: string; catalog: IndicatorCatalogItem[]; allowConstant?: boolean; allowedKinds?: string[]; compatibleGroups?: string[]; conditionKind?: string; positionSideConstant?: boolean }) {
  ...
  return <fieldset className="workflow-fieldset"><legend>{title}</legend>
    ...
    {value.kind === 'position' && <label><span>持仓字段</span><FormSelect value={value.name || 'profit'} onChange={(name) => onChange({ ...value, name })} data={[["side", "持仓方向"], ["profit", "当前盈亏"], ["open_price", "开仓价"], ["current_price", "当前价"], ["sl", "止损价"], ["tp", "止盈价"], ["volume", "持仓手数"], ["price_open_distance", "最新价 - 开仓价"], ["price_sl_distance", "最新价 - 止损价"], ["price_tp_distance", "最新价 - 止盈价"]]} /></label>}
...
function FormulaTargetFields({ target, onChange }: { target: NonNullable<NonNullable<WorkflowNode['action']>['target']>; onChange: (target: NonNullable<NonNullable<WorkflowNode['action']>['target']>) => void }) {
  if (target.kind !== 'formula') return null
  const patch = (updates: Partial<typeof target>) => onChange({ ...target, ...updates, kind: 'formula' })
  return <fieldset className={`workflow-fieldset workflow-action-rule workflow-formula-target${target.operation === 'none' ? ' workflow-formula-no-adjust' : ''}`}><legend>目标价格公式</legend><div className="workflow-field-row"><label><span>基础价格</span><FormSelect value={target.formula_base || 'current_price'} onChange={(formula_base) => patch({ formula_base: formula_base as NonNullable<typeof target>['formula_base'] })} data={[["current_price", "最新价"], ["entry_price", "开仓价"], ["sl", "原止损价"], ["tp", "原止盈价"], ["recent_low", "近期最低价"], ["recent_high", "近期最高价"]]} /></label><label><span>运算</span><FormSelect value={target.operation || 'subtract'} onChange={(operation) => patch({ operation: operation as 'none' | 'add' | 'subtract' })} data={[["add", "+"], ["subtract", "-"], ["none", "不调整"]]} /></label></div>{(target.formula_base === 'recent_low' || target.formula_base === 'recent_high') && <label><span>最近K线数量</span><input type="number" min="1" max="1000" value={target.lookback || 5} onChange={(event) => patch({ lookback: Number(event.target.value) })} /></label>}<div className="workflow-field-row"><label><span>调整方式</span><FormSelect value={target.adjustment_kind || 'atr'} onChange={(adjustment_kind) => patch({ adjustment_kind: adjustment_kind as 'fixed' | 'atr' })} data={[["fixed", "固定数值"], ["atr", "ATR倍数"]]} /></label><label><span>调整值</span><input type="number" min="0" step="0.1" value={target.adjustment_value ?? 1} onChange={(event) => patch({ adjustment_value: Number(event.target.value) })} /></label></div></fieldset>
}
...
function ActionFields({ node, stage, onChange }: { node: WorkflowNode; stage: WorkflowStageName; onChange: (node: WorkflowNode) => void }) {
  ...
  const setKind = (kind: WorkflowActionKind) => {
    ...
    if (kind === 'modify_sl' || kind === 'modify_tp') next.target = { kind: 'formula', formula_base: 'current_price', operation: 'subtract', adjustment_kind: 'atr', adjustment_value: 1 }
  ...
  return <>
    ...
    {needsTarget && <><label><span>目标价格</span><FormSelect value={action.target?.kind || 'formula'} onChange={(kind) => patchAction({ target: { ...action.target, kind: kind as NonNullable<NonNullable<WorkflowNode['action']>['target']>['kind'] } })} data={[["formula", "价格公式"]]} /></label>{['recent_low', 'recent_high'].includes(action.target?.kind || '') && <label><span>最近K线数</span><input type="number" min="1" max="1000" value={action.target?.lookback || 5} onChange={(event) => patchAction({ target: { ...action.target!, lookback: Number(event.target.value) } })} /></label>}{action.target?.kind === 'atr_offset' && <label><span>ATR倍数</span><input type="number" min="0.01" step="0.1" value={action.target.atr_multiplier || 0.5} onChange={(event) => patchAction({ target: { ...action.target!, atr_multiplier: Number(event.target.value) } })} /></label>}</>}
    {needsTarget && action.target?.kind === 'formula' && <FormulaTargetFields target={action.target} onChange={(target) => patchAction({ target })} />}{entryOffsetTarget}
    {opensPosition && <>
